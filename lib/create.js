...
var createJSON = function(options, callback) {
    ...
    switch (options.type) {
        ...
        case 'widget':
            ...
            fs.writeFileSync(path.join(skinBase, '../../', name + '-base.css'), '\n/**\n Your Code Goes Here\n*/\n', 'utf8');
            break;
    ...
    log.debug('creating HISTORYfile: ' + readme);
...
var createDirs = function(options, callback) {
    ...
    if (options.name.indexOf('gallery') === -1 && !util.isYUI()) {
        options.name = 'gallery' + ((options.type === 'css') ? 'css' : '') + '-' + options.name;
    }
...
exports.init = function(options, callback) {
    ...
    createDirs(options, function() {
        util.tree(options.location, options.root, function(tree) {
            ...
            log.log('');
            console.log(tree);
            log.info('yogi took ' + timer.calc(start, new Date().getTime()) + ' to create this module');
            callback();
        });
    });
};

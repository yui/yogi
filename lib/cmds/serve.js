...
mods = {
    ...
    fetchAndServe: function(file, res) {
        ...
        var self = this,
            _fetch = function() {
                ...
                if (self.options.parsed.yui && !self.isGallery(file)) {
                    url = 'http://yui.yahooapis.com/' + self.options.parsed.yui + '/' +  file;
                }
                log.debug('fetching: ' + url);

                proxy.request().get(url, function(err, r) {
                    if (r.statusCode === 200) {
                        res.charset = 'utf-8';
                        res.type(path.extname(String(file)));
                        res.header('YOGI-Remote-Fetch', 'In-Flight');
                        res.send(r.body);
                    } else {
    ...
    tests: function(callback) {
        ...
        this.app.options('/tests/'+'*', handler);
        this.app.use('/tests/'+'*', handler);
        ...
        this.app.get('/build/' + '*', function(req, res) {
            res.header('Yogi-Served', 'Yes');
            if (self.options.parsed.yui && !self.isGallery(req.params[0])) {
                return self.fetchAndServe(req.params, res);
            }
            ...
            var paths = [
                ...
                yuidocPath = path.join(self.root, '../', 'yuidocjs', 'node_modules/yui', req.params[0]);

            if (self.yui) {
                paths.push(path.join(self.yui, 'build/', req.params[0])); //yui build
            }

            if (util.exists(yuidocPath)) {
                paths.push(yuidocPath); //yuidoc internal
            }
            ...
            if (!served) {
                if (self.isGallery(req.params[0])) {
                    served = true;
    ...
    isGallery: function(file) {
        return (file.indexOf('gallery-') > -1 || file.indexOf('gallerycss-') > -1);
    },
    ...
    startServer: function() {
        ...
        if (this.options.grover && this.options.parsed.functional) {
            this.selleck(stack.add(noop));
        }
    ...
    help: function() {
        return [
            ...
            '--yui <version> The version of YUI to fetch from the CDN while testing'
        ];
    }
};

    ...
    tests: function(callback) {
        ...
        this.app.post('/tests/results', function (req, res) {
            ...
            var tmp = path.join(__dirname, '../../web/results/'),
                collect,
                report;
            log.debug('got istanbul results, generating report in ' + tmp);
            if (util.exists(tmp)) {
                rimraf.sync(tmp);
            }
            fs.mkdirSync(tmp);
        ...
        this.app.get('/tests/results/' + '*', function(req, res) {
            log.debug('serving istanbul coverage report');
        ...
        handler = function(req, res) {
            ...
            var file = path.join(base, 'tests', req.params[0]),
                file2 = path.join(base, req.params[0]);
            if (util.exists(file)) {
                fs.readFile(file, 'utf8', function(err, data) {
                    if (err) {
                        res.send('Not Found', 404);

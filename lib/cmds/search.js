        ...
        api.getNon('/gallery/search/?q=' + encodeURIComponent(q), function(err, json) {
            ...
            api.getNon('/cdn/buildtag', function(err, j) {
                if (json.count) {
                    ...
                    log.log('');
                    console.log('   ', pad('#', len).yellow, pad('user', uLen).yellow, pad('module', modLen).yellow, pad('slug', slugLen).yellow, ('since action'.yellow));
                    json.results.forEach(function(item) {
                        ...
                        console.log('   ', pad('#' + c, len)[color], pad(item.username, uLen), pad(item.module, modLen), pad(slug, slugLen), ('// ' + since)[stampColor]);
                    });

                    log.log('\n current build tag: ' + buildtag);
                    util.getCDNWindow(function(str) {
                        log.log(' ' + str);
                        log.log(' yogi completed the search in ' + timer.calc(START, new Date()));
                    });
                } else {

...
mods = {
    ...
    installNPM: function(callback) {
        log.info('installing npm dependencies in yui');
        ...
        child.on('exit', function() {
            log.info('npm install complete, continuing..');
            self.cli(callback);
        });
    ...
    cli: function(callback) {
        ...
        if (!util.exists(yuitestBin)) {
            log.error('Could not locate yuitest in: ' + yuitestBin);
            log.info('installing...');
            return this.installNPM(callback);
        }

...
mods = {
    ...
    meta: function() {
        ...
        if (util.exists(shifter)) {
            sJson = JSON.parse(fs.readFileSync(shifter, 'utf8'));
        }
        if (!util.exists(meta)) {
            ...
            api.get('/gallery/' + this.options.name + '/package.json', function(err, json) {
                ...
                if (sJson.builds[self.options.name] && sJson.builds[self.options.name].config) {
                    mod[self.options.name] = sJson.builds[self.options.name].config;
                    delete sJson.builds[self.options.name].config;
                    fs.writeFileSync(shifter, JSON.stringify(sJson, null, 4), 'utf8');
                }

...
mods = {
    ...
    sign: function() {
        ...
        var self = this,
            body = JSON.stringify({ body : ':thumbsup:' + ((this.signoff !== true) ? ' ' + this.signoff : '') });
    ...
    list: function() {
        ...
        this.getPulls(function(err, data) {
            ...
            data.forEach(function(item) {
                ...
                console.log(m, ('#' + pad(item.number)).yellow, '[' + item.user.login + ']', item.title, ('//last up: ' + (new Date(item.updated_at)).toDateString()).grey);
            });
        });

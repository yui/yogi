...
config.register('cdn-dirty', 'allow cdn requests with a dirty git root');
...
mods = {
        ...
        logs: function() {
            ...
            api.get('/cdn/logs/' + buildtag, function(err, json) {
                ...
                json.forEach(function(item) {
                    ...
                    if (item.failed) {
                        str += log.color(' (failed to build)', 'red');
                    }
                    log.log(str);
                });
        ...
        request: function() {
            ...
            git.status(function(status) {
                if (status && status.length && !self.force) {
                    log.bail('git root is dirty, please commit first or use: yogi cdn request --force');
                }
                var cmd = 'ghlog';
                ...
                git[cmd](function(logs) {
                    ...
                    if (sha && sha.sha) {
                        ...
                        log.bail('failed to locate commits for this module');
                    }
                });
            });
        },
        ...
        status: function() {
            ...
            api.get('/gallery/' + this.module.name + '/cdn', function(err, json) {
                ...
                if (json.action === 'pending') {
                    log.warn(log.color('CDN Request is still awaiting approval', 'yellow'));
                }
            ...
            api.get('/cdn/waiting', function(err, json) {
                ...
                if (json.count) {
                    ...
                    json.modules.forEach(function(item) {
                        ...
                        console.log('   ', log.color('#' + item.pullid, 'yellow'), item.sha1.substr(0, 7), item.module, item.user, log.color('// ' + since, 'white'));
            ...
            api.get('/cdn/queue', function(err, json) {
                ...
                if (json.count) {
                    ...
                    json.modules.forEach(function(item) {
                        ...
                        c++;
                        console.log('   ', log.color(pad('#' + c, len), 'yellow'), item.sha1.substr(0, 7), pad(item.module, modLen), slug, log.color('// ' + since, 'white'));
                    });
                }
                util.getCDNWindow(function(str) {
                    log.log('\n ' + str);
    ...
    init: function(options) {
        ...
        if (config.get('cdn-dirty')) {
            log.warn('allowing cdn requests with a dirty git root');
            this.force = config.get('cdn-dirty');
        }

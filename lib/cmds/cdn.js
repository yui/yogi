...
mods = {
    ...
    cmds: {
        logs: function() {
            var mod = this.module.name,
                buildtag = this.options.parsed.buildtag || 'latest';

            log.info('fetching cdn logs for ' + mod + ' from buildtag: ' + buildtag);
            ...
            api.get('/cdn/logs/' + buildtag, function(err, json) {
                ...
                json.forEach(function(item) {
                    log.log('   ' + pad(item.module) + '     ' + item.buildtag);
                });

...
mods = {
    ...
    getStatus: function(user, repo, callback) {
        ...
        request({
            ...
            if (res.body.last_build_status > 0) { //Failure
                request({
                    ...
                    console.log('       ', log.bad, sha, '(' + json.branch + ')', json.message,
                        '(' + json.author_name + ' <' + json.author_email + '>)');
                    json.matrix.forEach(function(m) {
                        var lang = m.config.language;
                        console.log('       ', ((m.result) ? log.bad : log.good), m.number, lang, m.config[lang]);
                    });
                    if (callback) {
                        callback();
                    }

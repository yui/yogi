    ...
    getStatus: function(user, repo, callback) {
        ...
        request({
            ...
            request({
                ...
                console.log('       ', ((json.status) ? log.bad : ((json.status === null) ? log.progress : log.good)), sha, '(' + json.branch + ')', json.message,
                    '(' + json.author_name + ' <' + json.author_email + '>)', '(' + json.state + ')');
                json.matrix.forEach(function(m) {
                    var lang = m.config.language;
                    console.log('           ', ((m.result) ? log.bad : ((m.result === null) ? log.progress : log.good)), m.number, lang, m.config[lang]);
                });
                if (callback) {
                    callback();
                }
